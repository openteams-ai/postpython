...
def parse_layout_signature(sig: str) -> UFuncSignature:
    """Parse a NumPy ufunc layout signature string."""
    from .compiler.frontend import parse_ufunc_sig as _parse
    return _parse(sig)
...
def _output_dtypes(
    ...
    """Infer NumPy dtypes for vectorized-function outputs from annotations."""
...
def _scalar_ufunc_signature(n_inputs: int) -> str:
    """Return the layout signature for an element-wise ufunc."""
    return ",".join("()" for _ in range(n_inputs)) + "->()"


def _signature_from_args(args: tuple[Any, ...], kwargs: dict[str, Any]) -> str | None:
    """Extract a NumPy layout signature from decorator arguments."""
    for arg in args:
...
def _broadcast_call(fn: Callable, sig: UFuncSignature, args: tuple) -> Any:
    """Execute *fn* with a pure-Python broadcast loop.

    This mirrors what the compiled ufunc wrapper does in C, for use when
    ...
    try:
        ...
        if output_args and len(output_args) != n_out:
            raise TypeError(
                f"ufunc expected {n_out} output argument(s), "
                f"got {len(output_args)}"
            )
        if kernel_expects_outputs and output_param_count != n_out:
            raise TypeError(
                f"ufunc kernel has {output_param_count} output parameter(s), "
        ...
        for arr, dims, nc in zip(arrays, sig.inputs, n_core_in):
            if arr.ndim < nc:
                raise ValueError(
                    f"ufunc input has {arr.ndim} dimensions, but signature "
        ...
        for dims in sig.outputs:
            try:
                ...
                raise ValueError(
                    f"cannot infer ufunc output core dimension {exc.args[0]!r}"
                ) from exc
        ...
        for i, core_shape in enumerate(output_core_shapes):
            ...
            if i < len(output_args):
                ...
                if out.shape != shape:
                    raise ValueError(
                        f"ufunc output {i} has shape {out.shape}, expected {shape}"
                    )
            else:
...
class GUVectorizeWrapper:
    ...
    __pp_ufunc_sig__ : parsed layout signature
    """
    ...
    def __init__(
        ...
        *,
        ...
        self.__pp_ufunc_sig__    = parse_layout_signature(sig_str)
    ...
    def __call__(self, *args: Any, **kwargs: Any) -> Any:
        ...
        return _broadcast_call(self.__wrapped__, self.__pp_ufunc_sig__, args)

    def __repr__(self) -> str:
        return f"<{self.__pp_decorator__} {self.__wrapped__.__name__!r} sig={self.__pp_sig__!r}>"

    def as_numpy_ufunc(self) -> Any:
        """Return a numpy.vectorize wrapper with the layout signature."""
        try:
            import numpy as np
...
class VectorizeWrapper(GUVectorizeWrapper):
...
def guvectorize(*args: Any, **kwargs: Any) -> Callable[[Callable], GUVectorizeWrapper]:
    ...
    if sig_str is None:
        raise TypeError("@guvectorize requires a NumPy layout signature")

    try:
        parse_layout_signature(sig_str)
    ...
    def decorator(fn: Callable) -> GUVectorizeWrapper:
        return GUVectorizeWrapper(
            fn,
            sig_str,
